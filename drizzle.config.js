...
const url = isProd ? (PROD_DATABASE_URL || DEV_DATABASE_URL) : (DEV_DATABASE_URL || PROD_DATABASE_URL);
console.log(`[Drizzle] Entorno de BD: ${isProd ? 'producción' : 'desarrollo'} (DB_ENV=${dbEnv})`);

if (!url) {
  throw new Error('No se encontró DEV_DATABASE_URL ni PROD_DATABASE_URL. Define una URL de conexión en .env.local');
}
